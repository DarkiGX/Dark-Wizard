...
const sim = ["sim", "Sim", "yes"];
const não = ["não", "Não", "no"];
 
const deleteCommandMessages = function(msg, client) {
  if (
    msg.deletable &&
    client.provider.get("global", "deletecommandmessages", false)
  ) {
    return msg.delete();
  }
};
  ...
  static async verify(channel, user, time = 30000) {
    const filter = res => {
      ...
      return (
        ...
        (sim.includes(value) || não.includes(value))
      );
    };
    ...
    if (sim.includes(choice)) return true;
    if (não.includes(choice)) return false;
    return false;
  }
}
